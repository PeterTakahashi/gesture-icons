...
## Installing an icon (shadcn-style CLI)

```bash
# once: the shared gesture engine (3 small files)
...
The registry is generated by `node scripts/gen-registry.mjs` + `npx shadcn build`
and served from `/r/*.json` on the demo site. Icons land in
`components/gesture-icons/icons/`, the engine in `components/gesture-icons/core/`.
The demo's detail view also offers static SVG snippets for Vue and plain HTML.
