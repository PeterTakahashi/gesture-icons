...
/**
 * Arrow up 01 — it makes its move. The arrow NUDGEs up on the base clock
 * while the two digits stamp in top-down order, an 8% beat apart — read
 * as the ascending order settling into place, top first.
 * Base geometry: Lucide `arrow-up-01` (ISC).
 */
const DUR = 0.85
...
export function ArrowUp01Icon({
  ...
  return (
    <svg
      ...
      className={className} style={style} role="img" aria-label={rest['aria-label'] ?? 'arrow up 01'}
      {...hoverProps}
    >
      <motion.g
        initial="normal"
