...
/**
 * Chevron last — the chevron nudges toward the bar it means to reach: a
 * small wind-up away, then the drive right toward the edge stop. The bar
 * takes a 1-unit contact nudge exactly on the frame the chevron arrives,
 * then both settle home.
 * Base geometry: Lucide `chevron-last` (ISC).
 */
const DUR = 0.85
...
export function ChevronLastIcon({
  ...
  return (
    <svg
      ...
      className={className} style={style} role="img" aria-label={rest['aria-label'] ?? 'chevron last'}
      {...hoverProps}
    >
      <motion.path
        d="m7 18 6-6-6-6"
...
export const meta = {
  ...
  section: 'Arrows',
  tags: ['navigate', 'step', 'chevron', 'last'],
}
